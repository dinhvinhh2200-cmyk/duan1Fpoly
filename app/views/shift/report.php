...
<div class="wrapper">
    ...
    <div id="content">
        ...
        <div class="container-fluid p-4">
            ...
            <h5 class="fw-bold text-secondary mb-3"><i class="fas fa-file-invoice-dollar me-2"></i> Hóa đơn đã thanh toán </h5>
            <div class="card shadow-sm border-0 mb-5">
                <div class="card-body p-0">
                    <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                        <table class="table table-hover align-middle mb-0">
                            <thead class="table-light sticky-top">
                                <tr>
                                    ...
                                    <th class="text-center">Giờ thanh toán</th>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="5" class="text-center py-4 text-muted fst-italic">
                                            Bạn chưa thanh toán đơn hàng nào trong ca này.
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
                            ...
                            <tbody>
                                <?php if(!empty($data['history'])): ?>
                                    <?php foreach($data['history'] as $s): ?>
                                        <?php 
                                            // [ĐÃ SỬA] CÔNG THỨC TÍNH CHÊNH LỆCH
                                            // Công thức cũ: Actual - (Opening + Total Sales)  <-- Sai vì Total Sales gồm cả CK
                                            // Công thức mới: Actual - (Opening + Cash Only)   <-- Đúng vì Két chỉ có Tiền mặt
                                            
                                            // Lấy tổng tiền mặt trong ca đó (từ subquery trong Model)
                                            // Nếu $s->cash_only chưa có (do dữ liệu cũ), mặc định là 0 để tránh lỗi
