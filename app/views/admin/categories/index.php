                                ...
                                <table class="table table-hover align-middle mb-0">
                                    <thead class="table-light sticky-top">
                                        <tr>
                                            ...
                                            <th>Tên danh mục</th>
                                            <th class="text-end pe-3">Hành động</th> </tr>
                                        </tr>
                                    </thead>
<tbody id="catTableBody">
    <?php if (!empty($data['categories'])): ?>
        ...
        <tr class="<?php echo ($cat->is_deleted == 1) ? 'table-secondary text-muted' : 'clickable-row'; ?>"
            <?php if($cat->is_deleted == 0): ?>
                onclick='editCategory(<?php echo htmlspecialchars(json_encode($cat), ENT_QUOTES, 'UTF-8'); ?>)'
                title="Click để sửa danh mục này"
            <?php endif; ?>
        >
            ...
            <td class="fw-bold">
                ...
                <?php if($cat->is_deleted == 1): ?>
                    <span class="badge bg-danger ms-2" style="font-size: 0.7rem;">Đã xóa</span>
                <?php endif; ?>
            </td>

            <td class="text-end pe-3">
                <?php if($cat->is_deleted == 1): ?>
                    <a href="<?php echo URLROOT; ?>/category/restore/<?php echo $cat->category_id; ?>" 
                       class="btn btn-sm btn-success fw-bold btn-restore"
